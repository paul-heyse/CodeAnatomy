...
def _prepare_pattern_state(ctx: QueryExecutionContext) -> PatternExecutionState | CqResult:
    ...
    if not file_result.files:
        ...
        if ctx.plan.explain:
            as_search_summary(result.summary).file_filters = _serialize_file_filter_decisions(
                file_result.decisions
            )
        return result
...
def _maybe_add_entity_explain(state: EntityExecutionState, result: CqResult) -> None:
    ...
    summary.file_filters = _serialize_file_filter_decisions(file_result.decisions)
...
def _maybe_add_pattern_explain(state: PatternExecutionState, result: CqResult) -> None:
    ...
    summary.file_filters = _serialize_file_filter_decisions(state.file_result.decisions)
