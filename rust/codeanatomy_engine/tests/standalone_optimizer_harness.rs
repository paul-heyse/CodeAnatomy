...
fn standalone_optimizer_harness_runs_with_default_rules() {
    ...
    let plan = ctx.read_empty().expect("read_empty").into_unoptimized_plan();
    let optimizer = Optimizer::new();
    let result = run_standalone_optimizer_harness(
        plan,
        optimizer.rules.clone(),
        4,
        true,
        true,
    )
...
fn standalone_optimizer_harness_supports_empty_rule_set() {
    let ctx = SessionContext::new();
    let plan = ctx.read_empty().expect("read_empty").into_unoptimized_plan();
    let result = run_standalone_optimizer_harness(plan, Vec::new(), 2, false, false)
        .expect("harness run");
