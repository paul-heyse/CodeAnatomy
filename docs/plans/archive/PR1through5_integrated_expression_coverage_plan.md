...
original PR-01..PR-04 scope with the additional expression-coverage scope items
we agreed on. It follows the same **PR-by-PR structure** as existing plans and adds:
...
target architecture defined in `docs/plans/Target_Architecture.md`. It also
adds a dedicated PR for expression coverage + SQL fallback to make the adapter
bridge durable.
...
This PR makes expression translation **complete and durable** by expanding
SQLGlot → DataFusion coverage, closing aggregate/join gaps, and adding a safe
SQL fallback with **parameter binding**. It also consolidates adapter entry
points so Ibis, SQLGlot, and DataFusion execution share one policy surface.

---

## Scope item 5.1 — Expand scalar expression coverage (arith/boolean/string/case)
...
def _case_expr(expr: exp.Case) -> Expr:
    ...
    for when in reversed(expr.args.get("ifs") or []):
        result = f.when(_expr_to_df(when.this), _expr_to_df(when.args["true"])).otherwise(result)
    return result
...
- [ ] Ensure handler mapping stays aligned with SQLGlot node types.
- [ ] Cast literals when DataFusion requires explicit typing.

---

## Scope item 5.2 — Aggregate + distinct coverage (COUNT*, FILTER, SELECT DISTINCT)
...
def _select_to_df(ctx: SessionContext, select: exp.Select) -> DataFrame:
    df = _select_to_df_base(ctx, select)
...
- [ ] Implement aggregate `FILTER` clauses with DataFusion filterable aggs.
...
- [ ] Strip qualifiers when DataFusion expects unqualified column keys.
- [ ] Detect ambiguous key names and fail fast with a TranslationError.
...
- [ ] Add a fallback helper that compiles SQLGlot → SQL.
- [ ] Ensure fallback uses DataFusion parameter binding.
...
- [ ] Thread parameter bindings through the DataFusion execution path.
- [ ] Align Ibis param names with DataFusion `param_values`.
- [ ] Preserve parameter typing in SQL rendering (no unsafe string literals).

---

## Scope item 5.6 — Consolidate adapter entrypoints (single policy surface)
...
- [ ] Route all Ibis → DataFusion translation through a single helper.
...
- [ ] Add unit tests for newly supported SQLGlot nodes.
- [ ] Add a fallback-path test for unsupported nodes.
- [ ] Gate tests on DataFusion availability in CI.

---

### Watchouts

* SQL fallback must preserve parameter types; avoid unsafe literal interpolation.
...
* Keep edits **strictly scoped** to PR-01..PR-05; do not change architecture.
