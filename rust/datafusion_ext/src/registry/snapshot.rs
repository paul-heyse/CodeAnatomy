...
pub fn snapshot_hook_capabilities(state: &SessionState) -> Vec<FunctionHookCapabilities> {
    ...
    for name in &snapshot.aggregate {
        let (groups_acc, retract) = function_metadata_for(name)
            .map(|meta| (meta.has_groups_accumulator, meta.has_retract_batch))
    ...
    for name in &snapshot.window {
        let (reverse, sort) = function_metadata_for(name)
            .map(|meta| (meta.has_reverse_expr, meta.has_sort_options))
            .unwrap_or((false, false));
        capabilities.push(FunctionHookCapabilities {
            name: name.clone(),
...
fn record_rewrite_tags(name: &str, snapshot: &mut RegistrySnapshot) {
    if let Some(metadata) = function_metadata_for(name) {
        if !metadata.rewrite_tags.is_empty() {
            snapshot.rewrite_tags.insert(
                name.to_string(),
                metadata
                    .rewrite_tags
                    .iter()
...
fn record_signature_flags(name: &str, signature: &Signature, snapshot: &mut RegistrySnapshot) {
    ...
    if let Some(metadata) = function_metadata_for(name) {
        if metadata.has_simplify {
            snapshot.simplify.insert(name.to_string(), true);
        }
    }
}

fn function_metadata_for(name: &str) -> Option<crate::registry::metadata::FunctionMetadata> {
    udf::function_metadata(name)
        .or_else(|| udaf_builtin::function_metadata(name))
        .or_else(|| udwf_builtin::function_metadata(name))
}
