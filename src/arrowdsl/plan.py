...
class Plan:
    ...
    def __post_init__(self) -> None:
        sources = [self.decl is not None, self.reader_thunk is not None, self.table_thunk is not None]
        if sum(sources) != 1:
    ...
    def project(self, expressions: Sequence["pc.Expression"], names: Sequence[str], *, label: str = "") -> "Plan":
        import pyarrow.acero as acero

        if self.decl is None:
            raise TypeError("project() requires an Acero-backed Plan (decl != None)")
        decl = acero.Declaration("project", acero.ProjectNodeOptions(list(expressions), list(names)), inputs=[self.decl])
        return Plan(decl=decl, label=label or self.label, ordering=self.ordering)
    ...
    def order_by(self, sort_keys: Sequence[Tuple[str, str]], *, label: str = "") -> "Plan":
        ...
        decl = acero.Declaration("order_by", acero.OrderByNodeOptions(sort_keys=list(sort_keys)), inputs=[self.decl])
        return Plan(decl=decl, label=label or self.label, ordering=Ordering.explicit(tuple(sort_keys)))
    ...
    def mark_unordered(self, *, label: str = "") -> "Plan":
        ...
        if self.table_thunk is not None:
            return Plan(table_thunk=self.table_thunk, label=label or self.label, ordering=Ordering.unordered())
        assert self.reader_thunk is not None
        return Plan(reader_thunk=self.reader_thunk, label=label or self.label, ordering=Ordering.unordered())
