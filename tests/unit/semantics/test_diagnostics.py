...
def test_semantic_quality_issue_batches_file_quality() -> None:
    ...
    batches = semantic_quality_issue_batches(
        view_name="file_quality_v1",
        df=ctx.table("file_quality_v1"),
        max_rows=10,
    )
    ...
    assert row["source_table"] == "file_quality_v1"
...
def test_semantic_quality_issue_batches_relationship_ambiguity() -> None:
    ...
    ctx.from_pydict(
        {
            "relationship_name": ["rel_name_symbol_v1"],
            "ambiguous_sources": [2],
        },
        name="relationship_ambiguity_report_v1",
    )
    batches = semantic_quality_issue_batches(
        view_name="relationship_ambiguity_report_v1",
        df=ctx.table("relationship_ambiguity_report_v1"),
        max_rows=5,
    )
    assert len(batches) == 1
    batch = batches[0]
    assert batch.issue_kind == "ambiguous_sources"
    assert batch.rows[0]["entity_id"] == "rel_name_symbol_v1"
