# DataFusion Rust UDF Best‑in‑Class Implementation Plan (v3)
...
**Intent**: Eliminate `ColumnarValue::values_to_arrays` hot‑paths by implementing explicit scalar/array handling. This is a primary perf win in DataFusion’s best‑practice guidance.
...
- [ ] Use `to_array_of_size` / `into_array_of_size` when arrayifying is unavoidable.
...
**Intent**: Make SQL‑defined macros fully discoverable through `SHOW FUNCTIONS` / `information_schema` by synthesizing `Documentation` + parameter names from the `CreateFunction` AST.
...
// Optional args (schema, delimiter, header, compression) -> ListingOptions
```
...
### Implementation checklist
- [ ] Support coercion of constant expressions into literals (e.g., `CAST(1 AS INT64)`).
- [ ] Add optional args for `read_csv/read_parquet` (schema hints, header, delimiter, compression).
...
### Implementation checklist
- [ ] Build `table_schema_overrides` from schema registry/dataset specs.
- [ ] Thread overrides into `datafusion_udf_specs(...)` call sites.
- [ ] Keep `struct<>` fallback when schema is unknown.
...
impl ScalarUDFImpl for MyUdf {
    ...
    fn return_type(&self, _arg_types: &[DataType]) -> Result<DataType> {
        Ok(DataType::Utf8) // consistent with return_field
    }
}
```
...
- [ ] Add helper for consistent return type derivation when needed.

---

## Scope 6 — Async UDF lane (feature‑gated, opt‑in)

**Intent**: Provide a production‑ready async UDF pathway for network/IO‑bound work without blocking execution threads.
...
- [ ] Ensure async UDFs are registered via `AsyncScalarUDF::new(...).into_scalar_udf()`.
- [ ] Add strict timeouts + bounded concurrency for any real async IO usage.

---

## Scope 7 — UDWF performance hooks (future‑proofing)

**Intent**: If we add any custom window UDFs, use the full performance‑oriented API (evaluate_all, bounded execution, memoize/prune, limit_effect).
...
**Intent**: Make sure all custom expr creation follows DataFusion’s coercion + rewrite pipeline to avoid subtle correctness regressions.
...
- Any custom planning path invoking physical expr creation

### Deletions
- None.

### Implementation checklist
- [ ] Audit any custom physical expr creation to use the SessionContext/SessionState entrypoints.
...
### Implementation checklist
- [ ] Add ABI handshake gates (`datafusion_ffi::version()` + plugin ABI version).
...
**Intent**: Remove code that cannot be safely deleted until the above scopes are proven complete.

### Candidates (do not delete until above scopes are green)
...
- Any “legacy” UDTF parsing paths if new coercion + options are fully deployed.

### Implementation checklist
- [ ] Confirm all UDFs are discoverable via `information_schema` and `udf_docs_snapshot`.
- [ ] Confirm all tests (Rust + Python) pass under strict info_schema enablement.
...
2) Scope 2 — FunctionFactory docs + named args
3) Scope 3 — UDTF coercion + options
...
8) Scope 8 — Expr planner/rewrite discipline
9) Scope 9 — Documentation automation
10) Scope 10 — FFI plugin alignment
11) Deferred decommissioning sweep
