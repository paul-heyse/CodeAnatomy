...
def _output_alias(row: ExtractMetadata) -> str:
...
def _bundle_templates(rows: tuple[ExtractMetadata, ...]) -> dict[str, str | None]:
    ...
    for row in rows:
        ...
        for bundle_name in bundles:
            templates.setdefault(bundle_name, set()).add(row.template)
    resolved: dict[str, str | None] = {}
    for bundle_name, values in templates.items():
        if len(values) == 1:
            resolved[bundle_name] = next(iter(values))
        else:
            resolved[bundle_name] = None
    return resolved


def _bundle_specs_from_rows(rows: tuple[ExtractMetadata, ...]) -> tuple[OutputBundleSpec, ...]:
    ...
    for row in rows:
        ...
        if row.template:
            bundles.add(f"{row.template}_bundle")
        for bundle_name in bundles:
            bundle_outputs.setdefault(bundle_name, []).append(output)
    for bundle_name, outputs in _DERIVED_OUTPUTS_BY_BUNDLE.items():
        bundle_outputs.setdefault(bundle_name, []).extend(outputs)
        for output in outputs:
            dataset_map.setdefault(output, None)
    templates = _bundle_templates(rows)
    specs: list[OutputBundleSpec] = []
    for bundle_name, outputs in bundle_outputs.items():
        ordering = _BUNDLE_ORDER_OVERRIDES.get(bundle_name, tuple(outputs))
        mapped = {output: dataset_map.get(output) for output in outputs}
        specs.append(
            OutputBundleSpec(
                name=bundle_name,
                outputs=tuple(outputs),
                template=templates.get(bundle_name),
                ordering=ordering,
                dataset_map=mapped,
            )
        )
...
_OUTPUT_BUNDLES: Mapping[str, OutputBundleSpec] = {
    spec.name: spec for spec in _bundle_specs_from_rows(tuple(extract_metadata_specs()))
}

_OUTPUT_TO_DATASET: dict[str, str] = {}
_OUTPUT_SKIP: set[str] = set()
_DATASET_NAMES: set[str] = {row.name for row in extract_metadata_specs()}
