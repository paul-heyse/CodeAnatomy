"""Tests for byte-seek navigation through structural export public APIs."""
...
EXPECTED_STRUCTURAL_NODE_COUNT = 2
...
class _Node:
    ...
    def walk(self) -> Any:
        return _Cursor(nodes=[self, _Node("identifier", 4, 8, (0, 4), (0, 8), tree=self.tree)])
...
def test_export_structural_rows_uses_byte_seek_child_index_zero(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Test export structural rows uses byte seek child index zero."""
    ...
    assert len(exported.nodes) == EXPECTED_STRUCTURAL_NODE_COUNT
