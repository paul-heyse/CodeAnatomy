...
| S2 | Complete | Dependency-direction closure is landed for core boundaries: remaining non-bootstrap type-coupling imports from `core` into `query/search` were removed (`run_context.py` no longer imports `query.sg_parser.SgRecord`, `services.py` no longer imports `search._shared.types.SearchLimits`), leaving cross-layer construction imports isolated to `core/bootstrap.py` composition only. |
...
| S8 | Complete | Decomposition closure is landed for the remaining search/core slices: Python ast/import tier ownership moved out of `extractors_runtime_core.py` into `extractors_runtime_ast.py`, `extractors_runtime_core.py` is now orchestration/dispatch focused, and `front_door_assembly.py` was reduced to focused front-door assembly helpers with nonessential facade re-export clutter removed. |
| S9 | Complete | Typed enrichment boundary cutover is landed for the primary smart-search path: classification now consumes typed extractor contracts directly via `enrich_python_contract_by_byte_range(...)`, and dict normalization/wrap bridging (`normalize_python_payload(...)` + `wrap_python_enrichment(...)`) was removed from the primary classification flow. Remaining dict-to-typed conversion points are explicit boundary adapters (extractor compatibility wrappers and semantic-plane payload ingestion), not primary pipeline execution paths. |
...
| S16 | Complete | Macros/query periphery decomposition is complete for planned slices: `macros/calls/entry.py` is now command-boundary-only, with scan/analyze/front-door/result orchestration owned by `entry_runtime.py`/`entry_output.py` and command execution in `entry_command.py`. |
...
| D5 | Complete | Typed-helper deletion batch is complete: primary-path mapping fallback helpers are removed from adapter/pipeline/object-resolution surfaces, and the legacy `python_enrichment_payload(...)` helper is decommissioned from active CQ code paths. |
...
- [x] S2. Dependency direction corrections (non-bootstrap core type-coupling imports into query/search modules removed; cross-layer composition remains bootstrap-only)
...
- [x] S8. God module decomposition — search backends and core (python ast/import tier extraction moved to focused runtime module and front-door assembly surface reduced to focused helpers)
- [x] S9. Typed enrichment payload migration (primary classification path now consumes typed Python enrichment contracts directly; dict normalization/wrapping removed from primary path)
...
- [x] S16. God module decomposition — macros + query periphery (`macros/calls/entry.py` is command-boundary-only with orchestration owned by extracted runtime/output modules)
...
- [x] D5. Cross-scope deletion batch (typed-helper cleanup closed: residual `python_enrichment_payload`/payload-view fallback usage removed from primary CQ paths)
