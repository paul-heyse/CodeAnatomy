...
MIN_RUNTIME_CONTRACT_VERSION = 3
MIN_UDF_TABLE_COUNT = 2


def _assert_substrait_runtime_contract() -> None:
...
def _assert_extension_entrypoints(extension: object) -> tuple[bool, bool]:
    ...
    return has_unified_runtime, has_modular_runtime


def _assert_delta_entrypoints() -> None:
    for entrypoint in (
...
def _assert_capabilities_payload(
    extension: object,
    *,
    ...
    if isinstance(runtime_contract, dict):
        ...
        assert version >= MIN_RUNTIME_CONTRACT_VERSION
    else:
    ...
    assert table_count >= MIN_UDF_TABLE_COUNT


def _assert_plugin_manifest_contract(extension: object) -> None:
...
def test_required_runtime_capabilities_available(require_native_runtime: None) -> None:
    ...
    _assert_substrait_runtime_contract()
    extension = importlib.import_module("datafusion._internal")
    has_unified_runtime, has_modular_runtime = _assert_extension_entrypoints(extension)
    _assert_delta_entrypoints()
    _assert_capabilities_payload(
        extension,
        has_unified_runtime=has_unified_runtime,
        has_modular_runtime=has_modular_runtime,
    )
    _assert_plugin_manifest_contract(extension)
