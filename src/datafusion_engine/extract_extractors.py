...
def _outputs_for_rows(rows: Sequence[ExtractMetadata]) -> tuple[str, ...]:
    return tuple(row.output_name() for row in rows)


def _optional_outputs(rows: Sequence[ExtractMetadata]) -> tuple[str, ...]:
...
_OUTPUT_TO_TEMPLATE: dict[str, str] = {
    row.output_name(): row.template for row in extract_metadata_specs() if row.template is not None
}
