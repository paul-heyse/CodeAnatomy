...
import datafusion_engine.arrow.abi as df_arrow_abi
import datafusion_engine.arrow.build as df_arrow_build
import datafusion_engine.arrow.encoding as df_arrow_encoding
import datafusion_engine.arrow.interop as df_arrow_interop
import datafusion_engine.encoding.policy as df_encoding_policy
...
QualityPlanSpec = _quality_metrics.QualityPlanSpec
concat_quality_tables = _quality_metrics.concat_quality_tables
empty_quality_table = _quality_metrics.empty_quality_table
quality_from_ids = _quality_metrics.quality_from_ids
quality_issue_rows = _quality_metrics.quality_issue_rows
record_quality_issue_counts = _quality_metrics.record_quality_issue_counts

type ComputeExpression = df_arrow_interop.ComputeExpression
type SchemaLike = df_arrow_interop.SchemaLike
type TableLike = df_arrow_interop.TableLike

DATASET_STATS_ENCODING_POLICY = df_arrow_encoding.EncodingPolicy(
    dictionary_cols=frozenset({"dataset_name"})
)

COLUMN_STATS_ENCODING_POLICY = df_arrow_encoding.EncodingPolicy(
...
def table_summary(table: TableLike) -> TableSummary:
    ...
    """
    sch_fp = df_identity.schema_identity_hash(table.schema)
    schema_fields = cast(
        "list[JsonDict]", df_arrow_abi.schema_to_dict(table.schema).get("fields", [])
    )
    return {
...
def dataset_stats_table(tables: Mapping[str, TableLike | None]) -> TableLike:
    ...
    for name, t in tables.items():
        if t is None:
            continue
        sch_fp = df_identity.schema_identity_hash(t.schema)
    ...
    table = df_arrow_build.rows_to_table(rows, DATASET_STATS_SCHEMA)
    return df_encoding_policy.NormalizePolicy(encoding=DATASET_STATS_ENCODING_POLICY).apply(table)
...
def column_stats_table(tables: Mapping[str, TableLike | None]) -> TableLike:
    ...
    table = df_arrow_build.rows_to_table(rows, COLUMN_STATS_SCHEMA)
    return df_encoding_policy.NormalizePolicy(encoding=COLUMN_STATS_ENCODING_POLICY).apply(table)
...
def empty_scan_telemetry_table() -> TableLike:
    ...
    """
    return df_arrow_build.empty_table(SCAN_TELEMETRY_SCHEMA)
...
def scan_telemetry_table(rows: Sequence[Mapping[str, object]]) -> TableLike:
    ...
    return df_arrow_build.rows_to_table(normalized, SCAN_TELEMETRY_SCHEMA)
