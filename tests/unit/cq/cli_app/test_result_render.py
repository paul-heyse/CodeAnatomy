...
def test_render_result_both_combines_markdown_and_json(monkeypatch) -> None:
    result = _result()
    monkeypatch.setattr("tools.cq.cli_app.result_render.render_markdown", lambda _r: "MD")
    monkeypatch.setattr("tools.cq.cli_app.result_render.dumps_json", lambda _r, indent=2: "JSON")
...
def test_emit_output_writes_json_like_directly(monkeypatch) -> None:
    sink = io.StringIO()
...
def test_emit_output_uses_console_print_for_markdown(monkeypatch) -> None:
    sink = io.StringIO()
    console = _Console(file=sink, printed=[])
    monkeypatch.setattr("tools.cq.cli_app.app.console", console)
