...
import pytest
from tools.cq.macros import scan_utils
...
def test_iter_files_applies_max_files(monkeypatch: pytest.MonkeyPatch, tmp_path: Path) -> None:
    ...
    monkeypatch.setattr(scan_utils, "resolve_macro_files", lambda **_kwargs: files)
    rows = scan_utils.iter_files(root=tmp_path, max_files=2)
    assert rows == files[:2]
