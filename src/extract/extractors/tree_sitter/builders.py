...
# Runtime implementation merged from builders.py.
...
from extract.extractors.tree_sitter.cache import TreeSitterParseResult
from extract.extractors.tree_sitter.visitors import (
    _NodeStats,
    _ParseContext,
    _ParseStats,
...
def _extract_ts_file_row_rust(
    ...
    *,
    ...
    try:
        try:
            batch_like = bridge(source, file_ctx.path, request_payload)
        except TypeError:
            batch_like = bridge(source, file_ctx.path)
    except (RuntimeError, TypeError, ValueError):
        return None
    if batch_like is None:
        return None
    if isinstance(batch_like, Mapping):
        # Stub/test bridges can return a fully materialized row payload.
        row_payload = {str(key): value for key, value in batch_like.items()}
        row_payload.setdefault("repo", options.repo_id)
        row_payload.setdefault("path", file_ctx.path)
        row_payload.setdefault("file_id", file_ctx.file_id)
        row_payload.setdefault("file_sha256", file_ctx.file_sha256)
        row_payload.setdefault("nodes", [])
        row_payload.setdefault("edges", [])
        row_payload.setdefault("errors", [])
        row_payload.setdefault("missing", [])
        row_payload.setdefault("captures", [])
        row_payload.setdefault("defs", [])
        row_payload.setdefault("calls", [])
        row_payload.setdefault("imports", [])
        row_payload.setdefault("docstrings", [])
        row_payload.setdefault("stats", None)
        row_payload.setdefault("attrs", {})
        return row_payload
    from datafusion_engine.arrow.coercion import to_arrow_table

    table = to_arrow_table(batch_like)
    ...
    for index, kind in enumerate(node_types):
        bstart = starts[index] if index < len(starts) else None
        bend = ends[index] if index < len(ends) else None
        if not isinstance(bstart, int) or not isinstance(bend, int):
            continue
        node_rows.append(
            {
                "id": f"{file_ctx.file_id}:{index}",
                "kind": str(kind),
                "span": span_dict(
                    SpanSpec(
                        start_line0=None,
...
def _extract_ts_file_row(
    ...
    *,
    ...
    if not _should_parse(file_ctx, options):
        return None
    rust_row = _extract_ts_file_row_rust(
        file_ctx,
        options=options,
        query_pack=query_pack,
    )
