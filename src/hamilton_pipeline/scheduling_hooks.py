...
class PlanTaskSubmissionHook(lifecycle_api.TaskSubmissionHook):
    ...
    def run_before_task_submission(  # noqa: PLR0913
        self,
        *,
        run_id: str,
        task_id: str,
        nodes: list[hamilton_node.Node],
        inputs: dict[str, object],
        overrides: dict[str, object],
        spawning_task_id: str | None,
        purpose: lifecycle_api.NodeGroupPurpose,
        **future_kwargs: object,
        ...
        nodes : list[hamilton_node.Node]
            Nodes included in the task submission.
        inputs : dict[str, object]
            Input values for the task group.
        overrides : dict[str, object]
            Overrides applied to the task group.
        spawning_task_id : str | None
            Parent task id if this group was spawned.
        purpose : lifecycle_api.NodeGroupPurpose
            Execution purpose for the task group.
        **future_kwargs : object
        ...
        """
        _ = overrides, spawning_task_id, future_kwargs
...
def plan_task_submission_hook(
    ...
    *,
    ...
    """
    return PlanTaskSubmissionHook(plan=plan, diagnostics=diagnostics, enforce_active=enforce_active)
...
def _execution_task_name(node_: hamilton_node.Node) -> str | None:
    tags = node_.tags
...
def _execution_task_names(nodes: Sequence[hamilton_node.Node]) -> tuple[str, ...]:
    names = [task_name for node_ in nodes if (task_name := _execution_task_name(node_))]
    return tuple(sorted(set(names)))


def _active_task_names(plan: ExecutionPlan, *, inputs: Mapping[str, object]) -> frozenset[str]:
    active_value = inputs.get("active_task_names")
    active: set[str] = set()
    if isinstance(active_value, (set, frozenset)) or (
        isinstance(active_value, Sequence) and not isinstance(active_value, (str, bytes))
    ):
        active = {name for name in active_value if isinstance(name, str)}
    if active:
