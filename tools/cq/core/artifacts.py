...
def save_artifact_json(
    ...
    Returns:
        Artifact: Artifact descriptor for the persisted file.
    """
...
def save_diagnostics_artifact(
    ...
    Returns:
        Artifact | None: Artifact descriptor if diagnostics payload existed, else
            ``None``.
    """
...
def save_search_artifact_bundle_cache(
    ...
    """Persist search artifact bundle in cache-backed retrieval store.

    Returns:
        Artifact | None: Artifact descriptor when persisted, else ``None``.
    """
    ...
    if persisted is None:
        return None
    return Artifact(path=f"cache://search_artifacts/{run_id}/{persisted.cache_key}", format="cache")
...
def list_search_artifact_index_entries(
    *,
    ...
    """
    return list_search_artifact_entries(
        root=root,
        run_id=run_id,
...
def load_search_artifact_bundle(
    *,
    ...
    """Load latest cached search artifact bundle for run id.
